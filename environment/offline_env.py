...
class OfflineEnv(gym.Env):
    ...
    def __init__(self, dataset_url=None, ref_max_score=None, ref_min_score=None, 
        ...
        ## [TODO BUG] it should be ok to delete kwargs, but pay attention!
    ...
    def get_dataset(self, h5path=None, no_check=False):
        ...
        ## checkdesign [BUG] skip sanity checks if eval
...
class PybulletEnv(gym.Env):
    ...
    def __init__(self, obs_low=-1., obs_high=1., action_low=-1., action_high=1.,):
        ...
        # self._set_action_space(np.array(action_low, dtype=np.float32), 
                            #    np.array(action_high, dtype=np.float32))
        
        # self._set_observation_space(np.array(obs_low, dtype=np.float32), 

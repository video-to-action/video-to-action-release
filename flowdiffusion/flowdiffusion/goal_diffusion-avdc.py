...
class Trainer(object):
    ...
    def sample(self, x_conds, batch_text, batch_size=1, guidance_weight=0):
        device = self.device
        task_embeds = self.encode_batch_text(batch_text) # [1, 4, 512]
        pdb.set_trace()
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step % self.save_model_freq == 0:
                        save_gap = self.train_num_steps // self.save_n_models
                        save_model_idx = (self.step // save_gap) * save_gap 
                        self.save(save_model_idx) # molestone
                        # assert save_model_idx <= self.save_n_models

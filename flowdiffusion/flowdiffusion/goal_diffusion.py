...
import sys
sys.path.append('/oscar/data/csun45/yluo73/r2024/AVDC_experiments/')
from diffuser.utils.luo_utils import batch_repeat_tensor
...
## Luo
from diffuser.models.helpers import get_no_dash_tasks_str
# ---------------
...
class GoalGaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, t, x_cond, task_embed,  clip_x_start=False, rederive_pred_noise=False):
        ...
        gw = self.guidance_weight
        use_cls_free = gw > 0.0
        if use_cls_free:
            # pdb.set_trace() # x_cond: [B, 3, 128, 128]
            x_in = torch.cat([x, x_cond], dim=1) ## [B, 24, 128, 128]
            ## t: [B,];  task_embed: [B, 4=n_words+2, 512]
            x_in_2, t_2, task_embed_2 = batch_repeat_tensor(x_in, t, task_embed, 2)
            task_embed_2[ len(t):, ... ] = 0.0
            out = self.model(   x_in_2, t_2, task_embed_2  )

            epsilon_cond = out[:len(t), ...]
            epsilon_uncond = out[len(t):, :, :]
            # pdb.set_trace()
            model_output = (1 + gw) * epsilon_cond - gw * epsilon_uncond
        else:
        ...
        ## this elif added by luo, Feb17
        elif self.objective == 'pred_v' and use_cls_free:
            v = epsilon_cond
            x_start = self.predict_start_from_v(x, t, v)
            x_start = maybe_clip(x_start)

            uncond_v = epsilon_uncond
            ...
            pred_noise = (1 + gw)*cond_noise - gw*uncond_noise
            x_start = self.predict_start_from_noise(x, t, pred_noise)
            # pdb.set_trace()
    ...
    def p_sample(self, x, t: int, x_cond, task_embed):
        ...
        ## NOTE: Low temperature sampling
        noise = noise * self.var_temp
    ...
    def sample(self, x_cond, task_embed, batch_size = 16, return_all_timesteps = False):
        # pdb.set_trace() ## channels here = 3*7
        image_size, channels = self.image_size, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        pred = sample_fn((batch_size, channels, image_size[0], image_size[1]), x_cond, task_embed,  return_all_timesteps = return_all_timesteps)
        ## img should be normed to [0, 1]
        return pred.clamp(min=0, max=1)
